...
const BookingTable = () => {
  ...
  const BookingHandler = (value) => {
    setPop(!isPop);
    setFetchUser(value);
    localStorage.setItem("mobilenumber",value.mobilenumber);
    localStorage.setItem("toDate",value.Date);
    localStorage.setItem("roomname",value.roomname);
  };
  var curr = new Date();
    ...
    const fetchUsers = async () => {
      try {
        const data = await sendRequest(
          ...
          "POST",
          JSON.stringify({ Date: toDate,roomname:room }),
          {
      ...
      {!isLoading && !isPop && (
        <div className="space-2 shadow mt-[2rem]">
          <h2 className="flex items-center justify-center text-[1rem] sm:text-[1.2rem] md:text-[3rem] text-black font-semibold">
            {`Booking Time Table for ${room} Room` }
          </h2>
