...
const EditBookingData = () => {
  ...
  const [originalDate, setOriginalDate] = useState([{ originalToDate: "" ,originalroomName:"",OriginalMobilenumber:""}]);
    ...
    const fetchUsers = async () => {
      try {
        ...
        if (success) {
          setUserData(user);
          setOriginalDate({
            originalToDate: user.toDate,
            originalroomName:user.roomname,
            OriginalMobilenumber:user.mobilenumber
          });
        } else {
          handleError(message);
        }
