...
module.exports.OTPLoginGenerator = async (req, res) => {
  try {
    ...
    try {
      ...
      console.log(otp);
      const existingOTPUser = await UserOTPVerification.findOne({ email });
      if (existingOTPUser) {
...
module.exports.verifyLoginOTP = async (req, res) => {
  try {
    ...
    try {
      ...
      const otpHolder = await UserOTPVerification.findOne({ email });
      if (!otpHolder) {
        return res
          .status(401)
...
module.exports.userDetail = async (req, res) => {
  try {
    const { mobilenumber, toDate, roomname } = req.body;
...
module.exports.deleteBooking = async (req, res) => {
  try {
    const { toHour, toDate, roomname } = req.body;
    console.log(req.body);
    const Timefilter = { toHour, Date: toDate, roomname };
    const filter = { toHour, toDate, roomname };
